 ...
 *  
 */
package org.mercury.cockpits.setup;
...
import org.mercury.cockpits.constants.MercuryCockpitsConstants;
 ...
 */
@SystemSetup(extension = MercuryCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(MercuryCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(MercuryCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						MercuryCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/mercurycockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
