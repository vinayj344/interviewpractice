...
import org.mercury.core.model.ApparelProductModel;
...
public class DefaultCheckoutFacade implements CheckoutFacade
{
	...
	/**
	 * @return the airpointService
	 */
	public AirpointService getAirpointService()
	{
		return airpointService;
	}

	/**
	 * @param airpointService
	 *           the airpointService to set
	 */
	public void setAirpointService(final AirpointService airpointService)
	{
		this.airpointService = airpointService;
	}
	...
	public List<CardTypeData> getSupportedCardTypes()
	{
		...
		return getCardTypeConverter().convertAll(cardTypes);
	}
	...
	{
		final CartModel cartModel = getCart();
		//final String productCode = getCart().getEntries().get(0).getProduct().getCode();
		int airpoints = 0;
		for (final AbstractOrderEntryModel ce : getCart().getEntries())
		{
			if (ce.getProduct() instanceof ApparelProductModel)
			{
				final ApparelProductModel appProduct = (ApparelProductModel) ce.getProduct();
				ce.getQuantity();
				airpoints = airpoints + (appProduct.getAirpoints().intValue() * ce.getQuantity().intValue());
			}
		...
		if (cartModel != null)
		{
			...
			if (cartModel.getUser().equals(currentUser) || getCheckoutCustomerStrategy().isAnonymousCheckout())
			{
				...
				//final int airpoint = getAirpointService().getAirpointService(productCode);
				if (currentUser instanceof CustomerModel)
				{
					final CustomerModel customer = (CustomerModel) currentUser;
					customer.setAirpoints(Integer.valueOf(airpoints));
